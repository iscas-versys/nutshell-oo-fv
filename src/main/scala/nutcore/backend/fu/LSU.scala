...
class moqEntry extends NutCoreBundle {
  ...
  val follower            = new PipelineFollower()
}
...
class LSU extends NutCoreModule with HasLSUConst {
  ...
  val havePendingDmemReq =
    MEMOpID.needLoad(moq(moqDmemPtr).op) && !moq(moqDmemPtr).loadPageFault && !moq(moqDmemPtr).storePageFault && !moq(
      moqDmemPtr
    ).loadAddrMisaligned && !moq(moqDmemPtr).storeAddrMisaligned && !moq(moqDmemPtr).finished && moq(
      moqDmemPtr
    ).valid && moq(moqDmemPtr).tlbfin
  val havePendingStoreEnq =
    MEMOpID.needStore(moq(moqDmemPtr).op) && !MEMOpID.needAlu(moq(moqDmemPtr).op) && moq(moqDmemPtr).valid && moq(
      moqDmemPtr
    ).tlbfin
  ...
  val skipAInst = !moq(moqDmemPtr).valid && moqDmemPtr =/= moqDtlbPtr || moq(moqDmemPtr).valid && moq(
    moqDmemPtr
  ).tlbfin && (moq(moqDmemPtr).loadPageFault || moq(moqDmemPtr).storePageFault || moq(
    moqDmemPtr
  ).loadAddrMisaligned || moq(moqDmemPtr).storeAddrMisaligned || !moq(moqDmemPtr).op(2, 0).orR)
  val haveLoadResp =
    io.dmem.resp.fire && MEMOpID.commitToCDB(opResp) && moq(
      moqidxResp
    ).valid // FIXIT: to use non blocking dcache, set it to false
  ...
  Debug(
    "[LSU MOQ] pc           id vaddr        paddr        func    op      data             mmio   valid   finished    exc     mask\n"
  )
  for (i <- 0 until moqSize) {
    Debug(
      ...
      moq(i).finished,
      moq(i).loadPageFault,
      moq(i).storePageFault,
      moq(i).loadAddrMisaligned,
      moq(i).storeAddrMisaligned,
      moq(i).brMask,
      i.U
    )
  ...
  val tlbRespStoreEnq =
    false.B // io.dtlb.resp.fire && MEMOpID.needStore(dtlbRespUser.op) && !MEMOpID.needAlu(dtlbRespUser.op) && !bruFlush && moq(dtlbRespUser.moqidx).valid && tlbRespmoqidx === moqDmemPtr
  storeQueueEnqueue := havePendingStoreEnq && !storeQueueFull || !havePendingDmemReq && tlbRespStoreEnq && !storeQueueFull
  val tlbRespAMOStoreEnq =
    false.B // io.dtlb.resp.fire && MEMOpID.needAlu(dtlbRespUser.op) && !bruFlush && moq(dtlbRespUser.moqidx).valid && tlbRespmoqidx === moqDmemPtr
  ...
  val storeQueueDequeue =
    storeQueueReqsend || storeQueueSkipInst // dmem.resp.fire && MEMOpID.commitToSTQ(opResp) && !MEMOpID.needLoad(opResp) //&& MEMOpID.needStore(opResp)
  ...
  // formal verification, but we can't access the Formal selection
  ...
  when(storeQueueEnqueue || storeQueueAMOEnqueue) {
    ...
    moq(storeQueueEnqSrcPick).follower.mem.write.memWidth := moq(storeQueueEnqSrcPick).size
    moq(storeQueueEnqSrcPick).follower.mem.write.valid    := true.B
    moq(storeQueueEnqSrcPick).follower.mem.write.addr     := moq(storeQueueEnqSrcPick).vaddr
    moq(storeQueueEnqSrcPick).follower.mem.write.data     := moq(storeQueueEnqSrcPick).data
  }
  ...
  val haveUnfinishedUCStore = if (EnableOutOfOrderMemAccess) { false.B }
  else { VecInit(storeQueue.map(i => i.isMMIO && i.valid)).asUInt.orR }
  ...
  Debug(
    storeQueueEnqueue || storeQueueAMOEnqueue,
    "[ENSTQ] pc %x moqidx %x valid %x enqp %x head %x\n",
    moq(moqidxResp).pc,
    moqidxResp,
    moq(moqidxResp).valid,
    storeQueueEnqPtr,
    storeHeadPtr
  )
  ...
  val pfType = Mux(
    havePendingDtlbReq,
    LSUOpType.needMemWrite(moq(moqDtlbPtr).func),
    LSUOpType.needMemWrite(func)
  ) // 0: load pf 1: store pf
  ...
  Debug(
    io.dtlb.resp.fire,
    "[DTLB RESP] %d: get paddr: %x for %x MOQid %x exc l %b s %b\n",
    GTimer(),
    io.dtlb.resp.bits.rdata,
    moq(tlbRespmoqidx).vaddr,
    tlbRespmoqidx,
    loadPF,
    storePF
  )
  ...
  // loadDMemReq
  val loadDMemReqSrcPick =
    Mux(havePendingDmemReq, moqDmemPtr, io.dtlb.resp.bits.user.get.asTypeOf(new DCacheUserBundle).moqidx)
  val loadDTlbRespReqValid =
    false.B // dtlbEnable && io.dtlb.resp.fire && MEMOpID.needLoad(dtlbRespUser.op) && !dmemReqFrommoq &&
  ...
  moq(loadDMemReqSrcPick).follower.mem.read.addr     := moq(loadDMemReqSrcPick).vaddr
  moq(loadDMemReqSrcPick).follower.mem.read.memWidth := moq(loadDMemReqSrcPick).size
  moq(loadDMemReqSrcPick).follower.mem.read.valid    := true.B
  ...
  // TODO: apply user
  dmem.req.bits.apply(
    addr = memReq.addr(VAddrBits - 1, 0),
    size = memReq.size,
    wdata = memReq.wdata,
    wmask = genWmask(memReq.addr, memReq.size),
    cmd = memReq.cmd,
    user = memReq.user.asUInt
  )
  ...
  val forwardVec = VecInit(List.tabulate(storeQueueSize)(i => {
    i.U < storeHeadPtr && io.dmem.req.bits.addr(PAddrBits - 1, log2Up(XLEN / 8)) === storeQueue(i)
      .paddr(PAddrBits - 1, log2Up(XLEN / 8)) && storeQueue(i).valid
  }))
  val forwardWmask = List
    .tabulate(storeQueueSize)(i => storeQueue(i).wmask & Fill(XLEN / 8, forwardVec(i)))
    .foldRight(0.U)((sum, i) => sum | i)
  for (j <- (0 to (XLEN / 8 - 1))) {
    dataBackVec(j) := MuxCase(
      // default = dmem.resp.bits.rdata(8*(j+1)-1, 8*j),
      default = 0.U,
      mapping = List
        .tabulate(storeQueueSize)(i => {
  ...
  Debug(
    io.dmem.req.fire && MEMOpID.needLoad(opReq),
    "[LSU] add load to MOQ pc:%x stmask:%x\n",
    io.dmem.req.bits.addr(PAddrBits - 1, 3),
    moq(reqRobidx).stMask
  )

  val storeNeedRollback = (0 until robSize)
    .map(i => {
      ldAddr(i) === Mux(
        havePendingStqEnq,
        moq(moqDmemPtr).paddr(PAddrBits - 1, 3),
        io.dtlb.resp.bits.rdata(PAddrBits - 1, 3)
      ) &&
  ...
  for (j <- (0 until (XLEN / 8))) {
    rdataFwdSelVec(j) := Mux(
      moq(moqidxResp).fmask(j),
      moq(moqidxResp).fdata(8 * (j + 1) - 1, 8 * j),
      dmem.resp.bits.rdata(8 * (j + 1) - 1, 8 * j)
    )
  }

  when(dmem.resp.fire) {
    when(MEMOpID.commitToCDB(opResp) || MEMOpID.commitToVPU(opResp)) {
      when(MEMOpID.needLoad(opResp)) {
        moq(moqidxResp).data                   := rdataFwdSel
        moq(moqidxResp).follower.mem.read.data := dmem.resp.bits.rdata
      }
  ...
  val atomDataReg =
    RegEnable(genWdata(atomALU.io.result, moq(moqidxResp).size), io.out.fire && LSUOpType.isAMO(moq(moqidxResp).func))
  ...
  io.out.bits := MuxCase(
    ...
    mapping = List(
      (moq(writebackSelect).loadPageFault || moq(writebackSelect).storePageFault || moq(
        writebackSelect
      ).loadAddrMisaligned || moq(writebackSelect).storeAddrMisaligned) -> moq(writebackSelect).vaddr,
  ...
  io.uopOut.decode.ctrl.rfWen := moq(writebackSelect).rfWen && !moq(writebackSelect).loadAddrMisaligned && !moq(
    writebackSelect
  ).storeAddrMisaligned && !moq(writebackSelect).loadPageFault && !moq(writebackSelect).storePageFault
  ...
  val reqpc = Mux(
    ...
    Mux(
      havePendingDmemReq,
      moq(moqDmemPtr).pc,
      Mux(dtlbEnable, moq(dtlbRespUser.moqidx).pc, moq(dtlbRespUser.moqidx).pc)
    )
  )
  Debug(
    "[DMEM] req v %x r %x addr %x data %x op %b id %x  resp v %x r %x data %x op %b id %x\n",
    dmem.req.valid,
    dmem.req.ready,
    dmem.req.bits.addr,
    dmem.req.bits.wdata,
    dmem.req.bits.user.get.asTypeOf(new DCacheUserBundle).op,
    dmem.req.bits.user.get.asTypeOf(new DCacheUserBundle).moqidx,
    dmem.resp.valid,
    dmem.resp.ready,
    dmem.resp.bits.rdata,
    dmem.resp.bits.user.get.asTypeOf(new DCacheUserBundle).op,
    dmem.resp.bits.user.get.asTypeOf(new DCacheUserBundle).moqidx
  )
  when(dmem.req.fire) {
    Debug("[DREQ] ")
    ...
    Debug(
      false,
      " pc %x addr 0x%x size %x wdata %x cmd %x moqidx %x memop %b spending %x lpending %x time %d\n",
      reqpc,
      dmem.req.bits.addr,
      dmem.req.bits.size,
      dmem.req.bits.wdata,
      ...
      memReq.user.op,
      storeCmtPtr - 0.U,
      moqHeadPtr - moqDmemPtr,
      GTimer()
    )
  }
  Debug(
    dmem.resp.fire,
    "[DRESP] memdata %x fwddata %x:%b data %x moqidx %x memop %b isMMIO %x time %d\n",
    dmem.resp.bits.rdata,
    moq(dmemUserOut.moqidx).fdata,
    moq(dmemUserOut.moqidx).fmask,
    rdataFwdSel,
    dmemUserOut.moqidx,
    dmemUserOut.op,
    lsuMMIO,
  ...
  Debug(
    dmem.req.fire && !MEMOpID.needStore(opReq) && forwardWmask.orR,
    "[FWD] dataBack %x forwardWmask %b\n",
    dataBack,
    forwardWmask
  )
  ...
  when(printMemTrace.B || addrTrack.map(i => dmem.req.bits.addr === i).foldRight(false.B)((sum, i) => sum | i)) {
    when(dmem.req.fire) {
      ...
      Debug(false, (!loadReadygo && !storeReadygo), "noDMemReq")
      Debug(
        " pc %x addr 0x%x size %x wdata %x cmd %x moqidx %x memop %b spending %x lpending %x\n",
        reqpc,
        dmem.req.bits.addr,
        dmem.req.bits.size,
        dmem.req.bits.wdata,
  ...
  Debug(
    dmem.resp.fire,
    "[LSU DRESP TRACE] memdata %x fwddata %x:%b data %x moqidx %x memop %b isMMIO %x\n",
    dmem.resp.bits.rdata,
    moq(dmemUserOut.moqidx).fdata,
    moq(dmemUserOut.moqidx).fmask,
    rdataFwdSel,
  ...
  Debug(
    dmem.req.fire && !MEMOpID.needStore(opReq) && forwardWmask.orR,
    "[LSU FWD TRACE] dataBack %x forwardWmask %b\n",
    dataBack,
    forwardWmask
  )
}
