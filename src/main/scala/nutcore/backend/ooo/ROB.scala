...
import rvspeccore.checker.{CheckerWithResult, CheckerWithWB, ConnectCheckerResult, ConnectCheckerWb}
import utils._
import difftest._
...
class ROB(implicit val p: NutCoreConfig)
  ...
  (0 until robWidth).map(i => {
    ...
    io.rcommited(2 * i) := commited(io.aprf(2 * i) >> 1)(io.aprf(2 * i)(0)) && valid(io.aprf(2 * i) >> 1)(
      io.aprf(2 * i)(0)
    )
    io.rcommited(2 * i + 1) := commited(io.aprf(2 * i + 1) >> 1)(io.aprf(2 * i + 1)(0)) && valid(
      io.aprf(2 * i + 1) >> 1
    )(io.aprf(2 * i + 1)(0))
  })
    ...
    for (j <- (0 to robWidth - 1)) {
      ...
      for (k <- (0 to robWidth - 1)) {
        when(valid(i)(j) && io.cdb(k).bits.prfidx === robIdx && io.cdb(k).valid) {
          ...
          Debug(
            commited(i)(j),
            "[ERROR] double commit robidx %d pc %x inst %x pcin %x instin %x\n",
            robIdx,
            decode(i)(j).cf.pc,
            decode(i)(j).cf.instr,
            io.cdb(k).bits.decode.cf.pc,
            io.cdb(k).bits.decode.cf.instr
          )
          Debug(
            io.cdb(k).bits.decode.cf.pc =/= decode(i)(j).cf.pc,
            "[ERROR] commit pc not match robidx %d pc %x inst %x pcin %x instin %x\n",
            robIdx,
            decode(i)(j).cf.pc,
            decode(i)(j).cf.instr,
            io.cdb(k).bits.decode.cf.pc,
      ...
      when(valid(i)(j) && needMispredictionRecovery(brMask(i)(j))) {
        ...
        canceled(i)(j) := true.B
        Debug(
          "[ROB] mis-prediction recovery invalidated robidx %d pc %x (%b)\n",
          robIdx,
          decode(i)(j).cf.pc,
          brMask(i)(j)
        )
  ...
  val tailBankNotUsed =
    List.tabulate(robWidth)(i => !valid(ringBufferTail)(i) || valid(ringBufferTail)(i) && commited(ringBufferTail)(i))
  val tailTermEmpty = List.tabulate(robWidth)(i => !valid(ringBufferTail)(i)).reduce(_ && _)
  // TODO: refactor retire logic
  val skipException =
    valid(ringBufferTail)(0) && redirect(ringBufferTail)(0).valid && commited(ringBufferTail)(0) && exception(
      ringBufferTail
    )(1)
    ...
    (0 until robWidth).map(i => {
      ...
      when(
        decode(ringBufferTail)(i).ctrl.rfWen &&
          rmtMap(decode(ringBufferTail)(i).ctrl.rfDest) === Cat(
            ringBufferTail,
            i.U(1.W)
          ) && // no other in flight inst will write this reg
      ...
      when(
        ...
        when(
          checkpoints(io.recoverCheckpoint.bits)
            .map(decode(ringBufferTail)(i).ctrl.rfDest) === Cat(ringBufferTail, i.U(1.W))
        ) {
  ...
  io.redirect.valid := retireATerm && List
    .tabulate(robWidth)(i => redirect(ringBufferTail)(i).valid && valid(ringBufferTail)(i))
    .reduce(_ || _)
  ...
  io.exception :=
    valid(ringBufferTail)(0) && exception(ringBufferTail)(0) ||
      valid(ringBufferTail)(1) && exception(ringBufferTail)(1) && (!valid(ringBufferTail)(0) || commited(
        ringBufferTail
      )(0) && !redirect(ringBufferTail)(0).valid)
  ...
  assert(
    !(exception(ringBufferTail)(0) && exception(ringBufferTail)(1) && valid(ringBufferTail)(0) && valid(ringBufferTail)(
      1
    ))
  )
  assert(
    !(exception(ringBufferTail)(0) && decode(ringBufferTail)(0).ctrl.fuType =/= FuType.lsu && valid(ringBufferTail)(0))
  )
  assert(
    !(exception(ringBufferTail)(1) && decode(ringBufferTail)(1).ctrl.fuType =/= FuType.lsu && valid(ringBufferTail)(1))
  )
  ...
  when(
    ...
    Debug("[ERROR] robpc1 %x robpc2 %x v %x lsupc %x\n", Q1.io.deq.bits, Q2.io.deq.bits, Q1.io.deq.valid, lsupc)
  }
  ...
  // ROB enqueue
  val validEnqueueRequest =
    List.tabulate(robWidth)(i => io.in(i).valid).reduce(_ | _) // io.in(0).valid || io.in(1).valid
  when(validEnqueueRequest && ringBufferAllowin) {
    ...
    (0 until robWidth).map(i => {
      ...
      load(ringBufferHead)(i) := io.in(i).bits.ctrl.fuType === FuType.lsu && LSUOpType.needMemRead(
        io.in(i).bits.ctrl.fuOpType
      )
      store(ringBufferHead)(i) := io.in(i).bits.ctrl.fuType === FuType.lsu && LSUOpType.needMemWrite(
        io.in(i).bits.ctrl.fuOpType
      )
    ...
    when(
      io.in(1).valid && io.in(1).bits.ctrl.rfWen && io.in(1).bits.ctrl.rfDest =/= 0.U && !(io
        .in(0)
        .valid && io.in(0).bits.ctrl.fuType === FuType.bru)
    ) {
  ...
  Debug(
    io.in(0).fire,
    "[DISPATCH1] pc = 0x%x inst %x wen %x wdst %x\n",
    io.in(0).bits.cf.pc,
    io.in(0).bits.cf.instr,
    io.in(0).bits.ctrl.rfWen,
    io.in(0).bits.ctrl.rfDest
  )
  Debug(
    io.in(1).fire,
    "[DISPATCH2] pc = 0x%x inst %x wen %x wdst %x\n",
    io.in(1).bits.cf.pc,
    io.in(1).bits.cf.instr,
    io.in(1).bits.ctrl.rfWen,
    io.in(1).bits.ctrl.rfDest
  )
  Debug(
    io.cdb(0).valid,
    "[COMMIT1] pc = 0x%x inst %x wen %x wdst %x wdata = 0x%x\n",
    io.cdb(0).bits.decode.cf.pc,
    io.cdb(0).bits.decode.cf.instr,
    io.cdb(0).bits.decode.ctrl.rfWen,
    io.cdb(0).bits.decode.ctrl.rfDest,
    io.cdb(0).bits.commits
  )
  Debug(
    io.cdb(1).valid,
    "[COMMIT2] pc = 0x%x inst %x wen %x wdst %x wdata = 0x%x\n",
    io.cdb(1).bits.decode.cf.pc,
    io.cdb(1).bits.decode.cf.instr,
    io.cdb(1).bits.decode.ctrl.rfWen,
    io.cdb(1).bits.decode.ctrl.rfDest,
    io.cdb(1).bits.commits
  )
  Debug(
    retireATerm && valid(ringBufferTail)(0),
    "[RETIRE1] pc = 0x%x inst %x wen %x wdst %x wdata %x mmio %x intrNO %x\n",
    decode(ringBufferTail)(0).cf.pc,
    decode(ringBufferTail)(0).cf.instr,
    io.wb(0).rfWen,
    io.wb(0).rfDest,
    io.wb(0).rfData,
    isMMIO(ringBufferTail)(0),
    intrNO(ringBufferTail)(0)
  )
  Debug(
    retireATerm && valid(ringBufferTail)(1),
    "[RETIRE2] pc = 0x%x inst %x wen %x wdst %x wdata %x mmio %x intrNO %x\n",
    decode(ringBufferTail)(1).cf.pc,
    decode(ringBufferTail)(1).cf.instr,
    io.wb(1).rfWen,
    io.wb(1).rfDest,
    io.wb(1).rfData,
    isMMIO(ringBufferTail)(1),
    intrNO(ringBufferTail)(1)
  )

  Debug() {
    Debug("[ROB] ")
    for (i <- 0 to (robSize - 1)) {
      when(valid(i)(0) && commited(i)(0)) { Debug(false, "c") }.elsewhen(valid(i)(0)) { Debug(false, "v") }.otherwise {
        Debug(false, "-")
      }
    }
    Debug("\n[ROB] ")
    ...
    for (i <- 0 to (robSize - 1)) {
      Debug(
        "[ROB] 0x%x %d %d %d %d %d   0x%x %d %d %d %d %d  " + i,
        decode(i)(0).cf.pc,
        valid(i)(0),
        commited(i)(0),
        canceled(i)(0),
        redirect(i)(0).valid && valid(i)(0),
        exception(i)(0),
        decode(i)(1).cf.pc,
        valid(i)(1),
        commited(i)(1),
        canceled(i)(1),
        redirect(i)(1).valid && valid(i)(1),
        exception(i)(1)
      )
  ...
  if (p.Formal && p.OOFormal) {
    ...
    when(select === 0.U) {
      ...
      checker.io.instCommit.valid := RegNext(retireMultiTerms)
    }
  ...
  if (!p.FPGAPlatform) {
    for (i <- 0 until RetireWidth) {
      ...
      difftest_commit.io.rfwen := RegNext(
        io.wb(i).rfWen && io.wb(i).rfDest =/= 0.U
      ) // && valid(ringBufferTail)(i) && commited(ringBufferTail)(i)
  ...
  // sim pref counter
  val retireBruInst = (retireATerm && (decode(ringBufferTail)(0).ctrl.fuType === FuType.bru || decode(ringBufferTail)(
    1
  ).ctrl.fuType === FuType.bru))
